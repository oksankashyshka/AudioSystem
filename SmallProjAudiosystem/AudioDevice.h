...
public:
	...
	Song getSong(const int& num_song) const { return vec_songs_[num_song]; }
	int getCurrSong() const { return num_curr_song_; }//number of current song

	void setCurrSong(const int& num_song)
	{
		if ((num_song >= 0) && (num_song < vec_songs_.size())) num_curr_song_ = num_song;
	}

	virtual void load(const std::string& adress, const int& quantity_songs) = 0;//
